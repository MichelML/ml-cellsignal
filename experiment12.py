...
def print_warmup_lr(engine):
    ...
    epoch = engine.state.epoch
    if epoch < 6 and epoch not in printed_warmup_epochs:
        printed_warmup_epochs.append(epoch)
        print(f'Warmup stage. Epoch {epoch} | LR: {optimizer.param_groups[0]["lr"]}')
    elif epoch == 6 and epoch not in printed_warmup_epochs:
        printed_warmup_epochs.append(epoch)
        print('Warmup stage is over. Starting Cosine Annealing Scheduler.')
